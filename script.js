...
function combinedByDateML() {
    // Folds the Noor Database mail-merge sends into the Master_Log daily totals
    // so "All Mails" reflects every campaign, not just Master_Log events.
    if (!NOOR_BY_DATE.length) return ML_BY_DATE;
    var map = {};
    ML_BY_DATE.forEach(function (d) {
        map[d.date] = map[d.date] || { date: d.date, del: 0, open: 0, click: 0, resp: 0, bounce: 0 };
        ['del', 'open', 'click', 'resp', 'bounce'].forEach(function (k) { map[d.date][k] += d[k] || 0; });
    });
    NOOR_BY_DATE.forEach(function (d) {
        map[d.date] = map[d.date] || { date: d.date, del: 0, open: 0, click: 0, resp: 0, bounce: 0 };
        ['del', 'open', 'click', 'resp', 'bounce'].forEach(function (k) { map[d.date][k] += d[k] || 0; });
    });
...
document.querySelectorAll('.tab').forEach(function (t) {
    t.addEventListener('click', function () {
        ...
        setTimeout(function () { renderAll(); if (typeof renderPending === 'function') renderPending(); if (typeof renderPendSales === 'function') renderPendSales(); }, 30);
    });
});
...
function renderSamples() {
    ...
    var todayStrSm = (function () { var d = new Date(); return d.getFullYear() + '-' + String(d.getMonth() + 1).padStart(2, '0') + '-' + String(d.getDate()).padStart(2, '0'); })();
    var todaySamples = SAMPLES.filter(function (s) { return s.date === todayStrSm; });
    var todayQty = todaySamples.reduce(function (a, s) { return a + (s.qty || 1); }, 0);
    set('smToday', todaySamples.length);
    set('smTodaySub', todayQty + ' units');
    set('smTodayCount', todaySamples.length + ' sent today');

    var clientCounts = {};
    sm.forEach(function (s) { if (s.client && s.client !== '—') clientCounts[s.client] = (clientCounts[s.client] || 0) + 1; });
    var clientArr = Object.entries(clientCounts).sort(function (a, b) { return b[1] - a[1]; });
    set('smClientsCount', clientArr.length + ' clients');
    var clientsListEl = document.getElementById('smClientsListEl');
    if (clientsListEl) {
        clientsListEl.innerHTML = clientArr.length ? clientArr.map(function (c) {
            return '<div style="display:flex;justify-content:space-between;font-size:12px;padding:7px 4px;border-bottom:1px solid rgba(255,255,255,.04);"><span style="color:#fff;">' + c[0] + '</span><span style="color:#a78bfa;font-family:monospace;">' + c[1] + ' req' + (c[1] !== 1 ? 's' : '') + '</span></div>';
        }).join('') : '<div style="color:#555;font-size:12px;text-align:center;padding:20px;">No clients in range</div>';
    }
    var todayListEl = document.getElementById('smTodayListEl');
    if (todayListEl) {
        todayListEl.innerHTML = todaySamples.length ? todaySamples.map(function (s) {
            return '<div style="display:flex;justify-content:space-between;gap:8px;font-size:12px;padding:7px 4px;border-bottom:1px solid rgba(255,255,255,.04);"><span style="color:#fff;max-width:50%;overflow:hidden;text-overflow:ellipsis;white-space:nowrap;" title="' + (s.product || '') + '">' + (s.product || '—') + '</span><span style="color:#ddd;overflow:hidden;text-overflow:ellipsis;white-space:nowrap;max-width:30%;">' + (s.client || '—') + '</span><span style="color:#a78bfa;font-family:monospace;flex-shrink:0;">x' + (s.qty || 1) + '</span></div>';
...
function renderPending() {
    ...
    set('pdWeekAvg', weekAvg + '/day'); set('pdMonthAvg', monthAvg + '/day');
...
window.addEventListener('load', function () {
    ...
    if (typeof SHEET_URL !== 'undefined' && SHEET_URL) {
        ...
        (function () {
            ...
            fetch(SHEET_URL + '?t=' + Date.now(), { cache: 'no-store', signal: ctrl.signal, redirect: 'follow' })
    ...
    if (typeof NOOR_URL !== 'undefined' && NOOR_URL) {
        ...
        (function () {
            ...
            function parseMergeHeaderDate(label) {
                ...
                if (!m) return '';
                var day = parseInt(m[1], 10), mon = MONTHS[m[2].slice(0, 3).toLowerCase()], yr = m[3];
